...
function clampLongTasks(start, end) {
  let clampedLongTasks = [];
...
const trackFID = async () => {
  onFID(({name, value, delta, id, attribution}) => {
    log.event(name, {
      ...
      debug_event: attribution.eventTime,
...
const trackLCP = async () => {
  onLCP(({name, value, delta, id, attribution}) => {
    ...
    console.log('TBT', getTBT(0, value));

    const ltt = getLTT(value - attribution.elementRenderDelay, value);

    console.log('resourceLoad', value - attribution.elementRenderDelay);
    console.log(ltt);
    console.log('LCP', value);

    console.log('%', Math.round(100 * ltt / attribution.elementRenderDelay));

    console.log(clampLongTasks(value - attribution.elementRenderDelay, value));
    ...
    log.event(name, {
      ...
      debug_rltt: getTBT(value - attribution.elementRenderDelay, value),
      event_id: id,
    });
  });
};
